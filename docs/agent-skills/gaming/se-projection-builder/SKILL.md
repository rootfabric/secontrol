...
linked_files:
  references:
    - references/blocks-vs-devices.md
...
```python
# Найти устройства — g.devices это dict[str, Device], ключ = строковый ID
for dev_id, dev in g.devices.items():
    dtype = type(dev).__name__
    print(f"  {dev_id}: {dtype}")

proj = g.devices['PROJECTOR_ID']    # доступ только по строковому ID
welder = g.devices['WELDER_ID']

# ⚠️ g.get_device('connector') возвращает None — всегда искать по ID!
# ⚠️ g.blocks это dict[int, BlockInfo], НЕ список — доступ по g.blocks[BLOCK_ID]
```

**Важно:** не все блоки зарегистрированы как устройства. `LargeShipMergeBlock`
появляется в `g.blocks` но НЕ в `g.devices` — нельзя включить/выключить через
API. Только блоки с `isDevice: True` в metadata попадают в `g.devices`.
...
**buildableBlocks=0 при remainingBlocks>0** — значит проекция показывает блоки,
но ни один нельзя приварить. Причина: спроецированные блоки не примыкают ни к
одному существующему блоку ("зависли в воздухе"). Это нормально для workflow
клонирования (см. раздел ниже) — нужно физически подвести корабль к проекции.
...
## Клонирование грида (workflow для дублирования корабля)

Когда нужно сделать копию корабля: экспорт текущего грида → отстыковка второго
корабля → загрузка блюпринта на проектор → строительство клона.

### Пошаговый workflow

1. **Экспорт блюпринта полного грида** (оба корабля стыкованы):
   ```python
   ...
   xml = snap['xml']
   ```
2. **Сохранить XML** — stripped (без ComponentContainer) + raw:
   ```python
   import re
   stripped = re.sub(r'<ComponentContainer>.*?</ComponentContainer>', '', xml, flags=re.DOTALL)
   stripped = stripped.replace('encoding="utf-16"', 'encoding="utf-8"')
   with open('clone-blueprint.sbc', 'w') as f:
       f.write(stripped)
   ```
3. **Отстыковать второй корабль** (вручную в игре или через API).
4. **Загрузить блюпринт на проектор**:
   ```python
   proj.load_blueprint_xml(stripped)
   time.sleep(3)
   # totalBlocks = ВСЕ блоки (оба корабля)
   # remainingBlocks = блоки от отстыкованного корабля
   # buildableBlocks = 0 ← нормально! Блоки "в воздухе"
   ```
5. **Подвести корабль к проекции** — чтобы merge-блок/коннектор соединился,
   и спроецированные блоки стали adjacent к существующим → `buildableBlocks > 0`.
6. **Включить наносборщик** → строит клон.

### Важные моменты

- **`remainingBlocks` = ровно половина `totalBlocks`** — ожидаемо, т.к. вторая
  половина уже есть на текущем корабле.
- **`buildableBlocks=0` после загрузки** — НЕ ошибка. Блоки второго корабля
  не примыкают к текущему гриду (стыковка разъединена). Нужно физически
  подвести корабль к проекции, чтобы блоки стали buildable.
- **Merge-блок (`LargeShipMergeBlock`) НЕ управляется через API** — он в `g.blocks`
  но не в `g.devices`. Включить/выключить можно только вручную в игре.
- **Коннектор** управляется через API (`g.devices[ID]`): `enable()`, `toggle_connect()`,
  `connect()`, `disconnect()` — но `nearbyConnectors` должен быть непустым.
- **include_connected=True** — обязательно для полного захвата всех блоков грида
  (включая стыкованные корабли).
...
12. **`g.blocks` — dict, не список** — `g.blocks[BLOCK_ID]` возвращает `BlockInfo`. Итерация: `for bid, binfo in g.blocks.items():`. `g.blocks[0]` → KeyError.
13. **Не все блоки — устройства** — `LargeShipMergeBlock`, `LargeBlockArmorBlock` и др. structural блоки есть в `g.blocks` но НЕ в `g.devices`. Нельзя включить/выключить через device API. `send_grid_command("enable", payload={"deviceId": merge_id})` тоже не работает — команда уходит (result=1) но не действует.
14. **`g.get_device('connector')` → None** — поиск по имени типа не работает. Только по строковому ID: `g.devices['88575364068781680']`.
15. **Коннектор: `toggle_connect()` требует nearbyConnectors** — если рядом нет другого коннектора, `connectorIsConnected` останется `False`.
...
```
5. **XML encoding="utf-16"** — заголовок не важен, игра принимает и utf-8 и utf-16 declarations. Но Python `xml.etree.ElementTree.parse()` выдаст `ParseError` при `encoding="utf-16"` на UTF-8 файле. Перед валидацией в Python: `xml.replace('encoding="utf-16"', 'encoding="utf-8"')`.
