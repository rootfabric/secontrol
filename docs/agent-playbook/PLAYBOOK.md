...
| [se-grid-creation](../agent-skills/gaming/se-grid-creation/SKILL.md) | Чек-лист и диагностика "оживления" нового/припаркованного грида: lock merge → заправка → enable_devices.py → аудит. Главные грабли: неполный PASSIVE_TYPES, batteries auto-disabled на обесточенном гриде, merge ≠ power sharing, динамические id при объединении. Использовать, когда "припарковал — а ничего не работает" или "батареи не включаются". | (read-only playbook, без скрипта) |
