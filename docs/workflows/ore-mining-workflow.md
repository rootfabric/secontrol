...
**⚠️ ЖЁСТКОЕ ПРАВИЛО:** Каждая руда перерабатывается ТОЛЬКО в слитки своего типа.
Gold получается только из Gold ore. Platinum — только из Platinum ore. Silver — только из Silver ore.
Никакой конвертации между разными рудами не существует. Если нужной руды нет на карте — скажи пользователю прямо, не выдумывай «переплавку» одной руды в другую.
