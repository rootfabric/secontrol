...
DEVICE_REGISTRY = {
    ...
    "MyObjectBuilder_LargeGatlingTurret": "gatling_turret",
    "MyObjectBuilder_InteriorTurret": "interior_turret",
    "MyObjectBuilder_LargeMissileTurret": "missile_turret",
    "MyObjectBuilder_SmallGatlingGun": "gatling_gun",
    "MyObjectBuilder_SmallMissileLauncher": "rocket_launcher",
    "MyObjectBuilder_SmallMissileLauncherReload": "reloadable_rocket_launcher",
...
class Grid:
    ...
    def __init__(
        ...
        try:
            ...
            # Redis ACL may allow regular keys but not keyspace channels.
            # The lazy boot_id check before commands still protects bindings.
            self._runtime_subscription = None
        try:
    ...
    def _on_grid_change(self, key: str, payload: Optional[Any], event: str) -> None:
        ...
        payload_name = payload.get("name") or payload.get("gridName") or payload.get("displayName")
        if payload_name:
            self.name = str(payload_name)
        ...
        if not device_metadata:
            # Summary gridinfo payloads may intentionally contain no devices.
            # Keep existing device objects alive so upper-level code keeps working
            # until the next full gridinfo refresh arrives.
            self.identity_suspect = False
        ...
        for old_device in old_devices.values():
            stable_key = self._device_stable_key(old_device.metadata)
            if stable_key:
                old_by_stable.setdefault(stable_key, old_device)

        new_devices: Dict[str, BaseDevice] = {}
        new_devices_by_num: Dict[int, BaseDevice] = {}
        new_devices_by_stable: Dict[str, BaseDevice] = {}
        reused_ids: set[int] = set()

        for metadata in device_metadata:
            device = old_devices.get(metadata.device_id)
            ...
            new_devices[metadata.device_id] = device
            reused_ids.add(id(device))

            try:
                new_devices_by_num[int(metadata.device_id)] = device
            except Exception:
                pass

            if stable_key:
                new_devices_by_stable[stable_key] = device

        for old_id, old_device in old_devices.items():
            if id(old_device) in reused_ids:
                continue
            try:
                old_device.close()
            except Exception:
                pass
            self.device_aliases.setdefault(str(old_id), str(old_id))

        self.devices = new_devices
        self.devices_by_num = new_devices_by_num
        self.devices_by_stable_key = new_devices_by_stable
        self.identity_suspect = False
        self.identity_suspect_reason = None
    ...
    def get_device(self, device_id: str) -> Optional["BaseDevice"]:
        ...
        if device is not None:
            return device
        alias = self.device_aliases.get(key)
        if alias:
            return self.devices.get(alias)
        self.ensure_current_binding()
        alias = self.device_aliases.get(key)
        if alias:
            return self.devices.get(alias)
    ...
    def get_device_any(self, device_id: int | str) -> Optional["BaseDevice"]:
        ...
        if isinstance(device_id, int):
            dev = self.devices_by_num.get(device_id)
            if dev is not None:
                return dev
        dev = self.get_device(str(device_id))
        if dev:
            return dev
        try:
            return self.devices_by_num.get(int(device_id))
    ...
    # ------------------------------------------------------------------
    def iter_blocks(self) -> Iterable[Any]:
        """Yield lightweight block entries from the latest gridinfo.

        Each yielded item has at least attribute ``block_id`` and ``raw`` (the
        original JSON dict). This mirrors the structure expected by examples
        that iterate blocks and send paint commands by ``block_id``.

        Falls back to fetching current gridinfo from Redis if local metadata is
        not initialized yet. If no blocks are present, yields nothing.
        """

        meta: Optional[Dict[str, Any]] = self.metadata
        if not isinstance(meta, dict):
            try:
                meta = self.redis.get_json(self.grid_key)  # type: ignore[assignment]
            except Exception:
                meta = None
        ...
        for entry in blocks:
            ...
            # Prefer standard key from telemetry ("id"), but accept common aliases
            raw_id = (
                entry.get("id")
            ...
            try:
                block_id = int(raw_id) if raw_id is not None else None
            except Exception:
                block_id = None

            if block_id and block_id > 0:
                yield SimpleNamespace(block_id=block_id, raw=entry)
    ...
    def send_grid_command(
        ...
        **extra: Any,
        ...
        sent = self.redis.publish(channel, message)
        if sent <= 0:
            self.mark_identity_suspect("grid command has no subscribers")
            if self.ensure_current_binding(force=True):
                fresh = dict(message)
                grid_int = _safe_int(self.grid_id)
                if grid_int is not None:
                    fresh["gridId"] = grid_int
                    fresh["gridEntityId"] = grid_int
                    fresh["grid_id"] = grid_int
                    fresh["targetId"] = grid_int
                    fresh["target_id"] = grid_int
                sent = self.redis.publish(self._grid_command_channel(), fresh)
        return sent
    ...
    def _extract_devices(self, payload: Dict[str, Any]) -> Iterable[DeviceMetadata]:
        ...
        for entry in candidates:
            ...
            for subtype_key in ("subtype", "subType", "Subtype"):
                if subtype_key in entry and entry.get(subtype_key) is not None:
                    extra.setdefault("subtype", entry.get(subtype_key))
                    break
            for position_key in ("position", "localPosition", "min", "gridPosition"):
                if position_key in entry and entry.get(position_key) is not None:
                    extra.setdefault(position_key, entry.get(position_key))
    ...
    def close(self) -> None:
        ...
        try:
            ...
            pass
        try:
            if self._runtime_channel_subscription is not None:
                self._runtime_channel_subscription.close()
        except Exception:
            pass
        ...
        self.devices_by_stable_key.clear()
...
class BaseDevice:
    ...
    def rebind(self, metadata: DeviceMetadata) -> None:
        old_key = self.telemetry_key
        self.metadata = metadata
        self.device_id = metadata.device_id
        self.device_type = metadata.device_type
        self.grid_id = metadata.grid_id
        ...
        if snapshot is not None:
            self._on_telemetry_change(self.telemetry_key, snapshot, "rebind")

    def update(self) -> Optional[Dict[str, Any]]:
        """Refresh telemetry from Redis and return the latest telemetry snapshot."""

        try:
            self.grid.before_command()
            self.grid.ensure_device_current(self)
        except Exception:
            pass
        ...
        if snapshot is not None:
            self._on_telemetry_change(self.telemetry_key, snapshot, "update")
    ...
    def wait_for_telemetry(
        ...
        *,
        ...
        """Wait until telemetry is available, optionally requiring a fresh sample."""

        deadline = time.monotonic() + max(0.0, float(timeout))
        interval = max(0.01, float(poll_interval))
        start_telemetry_at = self.last_telemetry_at

        if not wait_for_new and isinstance(self.telemetry, dict):
            return True
        ...
        while time.monotonic() <= deadline:
            ...
            if isinstance(self.telemetry, dict):
                if not wait_for_new or self.last_telemetry_at > start_telemetry_at:
                    return True

            time.sleep(interval)

        return isinstance(self.telemetry, dict) and not wait_for_new
    ...
    def send_command(self, command: Dict[str, Any]) -> int:
        ...
        def prepare_payload(source: Dict[str, Any]) -> Dict[str, Any]:
            payload = dict(source)
            did = to_int(self.device_id)
            gid = to_int(self.grid.grid_id)
            ...
            if did is not None:
                ...
                payload["entityId"] = did
                payload["targetId"] = did
                payload["target_id"] = did
            if gid is not None:
                payload["gridId"] = gid
                payload["gridEntityId"] = gid
                payload["grid_id"] = gid
            if pid is not None:
            ...
            if isinstance(meta, dict):
                ...
                payload["meta"] = {"user": "grid-wrapper"}

            now_ms = int(time.time() * 1000)
            payload.setdefault("seq", now_ms)
            payload.setdefault("ts", now_ms)
            return payload

        def publish_payload(payload: Dict[str, Any]) -> int:
            sent_count = 0
            for ch in self._command_channels():
                sent_count += self.redis.publish(ch, payload)
                print(payload)
            return sent_count
        ...
        payload = prepare_payload(original_command)
        sent = publish_payload(payload)

        if sent <= 0:
            self.grid.mark_identity_suspect("device command has no subscribers")
            self.grid.ensure_current_binding(force=True)
            self.grid.ensure_device_current(self, force=True)
            payload = prepare_payload(original_command)
            sent = publish_payload(payload)
...
TYPE_ALIASES = {
    ...
    "gatlingturret": "gatling_turret",
    "gatling_turret": "gatling_turret",
    ...
    "reloadablerocketlauncher": "reloadable_rocket_launcher",
    "reloadable_rocket_launcher": "reloadable_rocket_launcher",
    "lamp": "lamp",
    "light": "lamp",
    "lighting_block": "lamp",
    "interior_light": "lamp",
