...
class OreDetectorDevice(BaseDevice):
    ...
    def __init__(self, grid, metadata) -> None:  # noqa: D401 - BaseDevice initialises common state
        ...
        self._visible_markers: Dict[str, Any] = {}
    ...
    def handle_telemetry(self, telemetry: Dict[str, Any]) -> None:
        ...
        if isinstance(visible_markers, dict):
            self._visible_markers = dict(visible_markers)
            items = visible_markers.get("items")
            self._markers = [item for item in items if isinstance(item, dict)] if isinstance(items, list) else []
        elif isinstance(radar, dict) and isinstance(radar.get("markers"), list):
            self._visible_markers = {
                "ready": True,
                "items": [item for item in radar.get("markers", []) if isinstance(item, dict)],
            }
            self._markers = list(self._visible_markers["items"])
        # Do not reset radar data if radar/markers are not present in telemetry
    ...
    def visible_markers_snapshot(self) -> Dict[str, Any]:
        """Return the latest server-side visible marker scan snapshot."""
        return dict(self._visible_markers or {})

    def markers(self) -> List[Dict[str, Any]]:
        """Return markers from the latest marker scan."""
        return list(self._markers)

    def marker_revision(self) -> Optional[int]:
        markers = self._visible_markers or {}
        try:
            return int(markers.get("revision"))
        except (TypeError, ValueError):
            return None
    ...
    def scan_markers(
        ...
        *,
        ...
        """Request a server-side scan of visible GPS/HUD marks, strong signals and beacon/antenna broadcasts."""
        state: Dict[str, Any] = {
