...
class RadarController:
    ...
    def scan_markers(
        ...
        *,
        ...
        """Scan visible GPS/HUD marks, strong signals, beacons and antenna broadcasts.
        ...
        if hasattr(self.radar, "marker_revision"):
            initial_rev = self.radar.marker_revision()

        seq = self.radar.scan_markers(
            radius=radius,
        ...
        while time.time() - start < timeout:
            self.radar.update()
            if self.radar.wait_for_telemetry(timeout=0.5, wait_for_new=False, need_update=False):
                snapshot = self.radar.visible_markers_snapshot()
                try:
                    rev = int(snapshot.get("revision"))
                except (TypeError, ValueError):
                    rev = None
                if snapshot.get("ready") and (initial_rev is None or rev != initial_rev):
                    break
            time.sleep(0.1)
        ...
        strong = [m for m in markers if m.get("type") == "strong_signal"]
        beacons = [m for m in markers if m.get("type") == "beacon"]
        antennas = [m for m in markers if m.get("type") == "antenna"]
        gps = [m for m in markers if m.get("type") == "gps"]
        print(
            "Markers found: "
            f"total={len(markers)}, strong={len(strong)}, gps={len(gps)}, "
            f"beacons={len(beacons)}, antennas={len(antennas)}"
        )

        return snapshot if return_snapshot else markers
