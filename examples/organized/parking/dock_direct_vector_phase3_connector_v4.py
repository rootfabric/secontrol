...
# Settings
GYRO_GAIN = float(os.getenv("SE_DOCK_GYRO_GAIN", "0.18"))
MAX_RATE = float(os.getenv("SE_DOCK_MAX_GYRO_RATE", "0.12"))
...
APPROACH_SPEED = float(os.getenv("SE_DOCK_APPROACH_SPEED", "6.0"))
...
CONNECTOR_ROLL_WEIGHT = float(os.getenv("SE_DOCK_CONNECTOR_ROLL_WEIGHT", "0.30"))
...
def correct_orientation(rc, sc, gyros, axis_dir, timeout=8, tc=None):
    """Align the *ship connector* with the target connector frame.

    Old versions chased the live connector-to-connector line and commanded only
    pitch/yaw. That could stall around 10-20 degrees or make the ship spin in
    place. This version uses connector frame telemetry directly:
      desired ship connector forward = -target_connector.forward
      desired ship connector up      =  target_connector.up
    The gyro command is a world rotation vector projected into the ship body
    axes, with a small rate and slew limit.
    """
    start = time.time()
    last_print = 0.0
    ...
    while time.time() - start < timeout:
        refresh_devices(rc, sc, *( [tc] if tc is not None else [] ), delay=0.12)

        if tc is not None:
            err = connector_frame_error(sc, tc=tc)
        else:
            # Fallback for old behavior if target connector telemetry is missing.
            if axis_dir is None or dot(axis_dir, axis_dir) <= 1e-8:
                sc_pos = get_pos(sc.telemetry or {})
                tc_pos = get_pos(tc.telemetry or {}) if tc is not None else None
                if sc_pos and tc_pos:
                    axis_dir = normalize(vec_sub(tc_pos, sc_pos))
            err = connector_frame_error(sc, fallback_axis=axis_dir)

        if err is None:
            time.sleep(0.15)
            continue

        angle_err, fwd_err, roll_err, error_world, _desired_fwd, _desired_up = err
        best_angle = min(best_angle, fwd_err)

        if fwd_err < ALIGN_TOLERANCE and roll_err < math.radians(6.0):
            stable_ticks += 1
            if stable_ticks >= 5:
                for g in gyros:
                    g.clear_override()
        ...
        if dot(ship_fwd, ship_fwd) <= 1e-8 or dot(ship_up, ship_up) <= 1e-8 or dot(ship_right, ship_right) <= 1e-8:
            time.sleep(0.15)
            continue

        # If the angle consistently gets worse, invert all gyro axes once.
        # This is safer than telling the user to guess sign env vars.
        if last_angle is not None and fwd_err > last_angle + math.radians(0.35):
            worse_ticks += 1
        else:
            worse_ticks = max(0, worse_ticks - 1)
        last_angle = fwd_err
        if worse_ticks >= 4 and not flipped_once:
            sign_mul *= -1.0
            flipped_once = True
            worse_ticks = 0
            prev_pitch = prev_yaw = prev_roll = 0.0
            for g in gyros:
                g.clear_override()
            print("    align: gyro response looked inverted; flipping gyro sign once")
            time.sleep(0.5)
            continue

        rate = min(MAX_RATE, max(0.008, angle_err * GYRO_GAIN))
        raw_pitch = max(-rate, min(rate, dot(error_world, ship_right) * GYRO_GAIN * GYRO_PITCH_SIGN * sign_mul))
        raw_yaw = max(-rate, min(rate, dot(error_world, ship_up) * GYRO_GAIN * GYRO_YAW_SIGN * sign_mul))
        raw_roll = max(-rate, min(rate, dot(error_world, ship_fwd) * GYRO_GAIN * GYRO_ROLL_SIGN * sign_mul))

        max_delta = float(os.getenv("SE_DOCK_GYRO_SLEW", "0.010"))
        pitch_cmd = prev_pitch + max(-max_delta, min(max_delta, raw_pitch - prev_pitch))
        yaw_cmd = prev_yaw + max(-max_delta, min(max_delta, raw_yaw - prev_yaw))
        roll_cmd = prev_roll + max(-max_delta, min(max_delta, raw_roll - prev_roll))
        prev_pitch, prev_yaw, prev_roll = pitch_cmd, yaw_cmd, roll_cmd
        ...
        if elapsed - last_print >= 2.0:
            print(
                f"    align: fwd={math.degrees(fwd_err):.2f}° roll={math.degrees(roll_err):.2f}° "
                f"gyro=({pitch_cmd:+.3f},{yaw_cmd:+.3f},{roll_cmd:+.3f})"
            )
            last_print = elapsed
    ...
    for g in gyros:
        g.clear_override()
    refresh_devices(rc, sc, *( [tc] if tc is not None else [] ), delay=0.1)
    err = connector_frame_error(sc, tc=tc) if tc is not None else connector_frame_error(sc, fallback_axis=axis_dir)
    if err is None:
        return best_angle
    return err[1]
...
DIRECT_MAX_GYRO = float(os.getenv("SE_DOCK_DIRECT_MAX_GYRO", "0.08"))
    ...
    try:
        while time.time() - started < timeout:
            ...
            if frame and hold_frame:
                _hold_err, pitch, yaw, roll = dv_apply_orientation_control(gyros, frame, hold_frame)
            else:
                _hold_err, pitch, yaw, roll = 0.0, 0.0, 0.0, 0.0
            angle_err = connector_angle_err if docking_mode else _hold_err
...
_init_err = connector_frame_error(sc, tc=tc)
if _init_err is None:
    print("ERROR: cannot read connector orientation frame"); sys.exit(1)
init_angle = _init_err[1]
init_roll = _init_err[2]
print(f"  Initial connector frame error: forward={math.degrees(init_angle):.1f}° roll={math.degrees(init_roll):.1f}°")

if init_angle > ALIGN_TOLERANCE:
    final_angle = correct_orientation(rc, sc, gyros, axis_dir, timeout=90, tc=tc)
    print(f"  Final angle: {math.degrees(final_angle):.1f}°")
    if final_angle > ALIGN_TOLERANCE:
        print("  Second precision alignment pass...")
        final_angle = correct_orientation(rc, sc, gyros, axis_dir, timeout=60, tc=tc)
    ...
    if final_angle > math.radians(PHASE3_REQUIRED_ALIGN_DEG):
        ...
        print("TIP: reduce SE_DOCK_GYRO_GAIN / SE_DOCK_MAX_GYRO_RATE or check gyro sign mapping.")
        sys.exit(1)
else:
...
while True:
    ...
    if angle_err > ALIGN_TOLERANCE:
        ...
        final_angle = correct_orientation(rc, sc, gyros, axis_dir, timeout=45, tc=tc)
        for g in gyros:
            g.clear_override()
        time.sleep(0.3)
        refresh_devices(rc, sc, tc, delay=0.1)
