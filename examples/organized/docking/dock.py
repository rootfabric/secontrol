...
def parse_args():
    ...
    # Phase 3 fast parking approach. Far from the connector we can safely move
    # in larger hops and at higher speed; the final 20m still use the precise
    # slow logic.
    parser.add_argument("--phase3-fast-until", type=float, default=20.0, help="Use fast Phase 3 movement while axial connector distance is above this value, meters")
    parser.add_argument("--phase3-fast-step", type=float, default=40.0, help="Fast Phase 3 hop length before the final 20m zone, meters")
    parser.add_argument("--phase3-fast-speed", type=float, default=12.0, help="Fast Phase 3 movement speed before the final 20m zone, m/s")
    parser.add_argument("--phase3-fast-lineup-speed", type=float, default=8.0, help="Fast lateral line-up speed before the final 20m zone, m/s")
...
PHASE3_FAST_UNTIL_DISTANCE = max(5.0, float(ARGS.phase3_fast_until))
PHASE3_STEP_FAST = max(1.0, float(ARGS.phase3_fast_step))
PHASE3_STEP_SLOW = 3.0
PHASE3_STEP_NEAR = 2.0
PHASE3_STEP_CREEP = 0.75
PHASE3_STEP_FINAL = 0.25

PHASE3_SPEED_FAST = max(0.5, float(ARGS.phase3_fast_speed))
PHASE3_LINEUP_SPEED_FAST = max(0.5, float(ARGS.phase3_fast_lineup_speed))
...
print(
    f"  Fast Phase 3: until axial>{PHASE3_FAST_UNTIL_DISTANCE:.1f}m, "
    f"step={PHASE3_STEP_FAST:.1f}m, speed={PHASE3_SPEED_FAST:.1f}m/s, "
    f"lineup_speed={PHASE3_LINEUP_SPEED_FAST:.1f}m/s"
)
...
while True:
    ...
    if signed_axial > PHASE3_FAST_UNTIL_DISTANCE:
        phase = "FAST"
    ...
    if lateral_error > lateral_limit:
        ...
        lineup_speed = PHASE3_LINEUP_SPEED_FAST if signed_axial > PHASE3_FAST_UNTIL_DISTANCE else min(speed, 1.0)
